...
import bl_utils.comfyui_version
import bl_utils.hook_breaker_ac10a0
...
def cuda_malloc_warning():
    ...
    if "cudaMallocAsync" in device_name:
        for b in bl_utils.cuda_malloc.blacklist:
            if b in device_name:
...
def start_flowengine(asyncio_loop=None):
    ...
    prompt_server = server.PromptServer(asyncio_loop, bl_mode=True)

    bl_utils.hook_breaker_ac10a0.save_functions()
    asyncio_loop.run_until_complete(bl_nodes.nodes.init_extra_nodes(
        init_custom_nodes=(not args.disable_all_custom_nodes) or len(args.whitelist_custom_nodes) > 0,
        init_api_nodes=not args.disable_api_nodes
    ))
    bl_utils.hook_breaker_ac10a0.restore_functions()
...
if __name__ == "__main__":
    setup_env_variable()
    import bl_utils.cuda_malloc

    # Running directly, just start ComfyUI.
    logging.info("Python version: {}".format(sys.version))
    logging.info("ComfyUI version: {}".format(bl_utils.comfyui_version.__version__))
